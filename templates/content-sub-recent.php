<article id="post-<?php the_ID() ?>" <?php post_class('sub-recent-post') ?> itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="Blog">
	<div class="row pl-0 pr-0">
		<div class="col-12 col-xl-6 cats-pr">
			<figure class="article-sub-recent-thumb m-0" itemprop="image" itemscope itemType="http://schema.org/ImageObject">
		    	<a href="<?php the_permalink(); ?>" itemprop="url">
		     		<link itemprop="url" href="<?php the_post_thumbnail_url(); ?>">
		        	<img class="responsive-img sub-recent-img" src="<?php if(wp_get_attachment_url( get_post_thumbnail_id() )){ echo wp_get_attachment_url( get_post_thumbnail_id() ); }else{ echo get_template_directory_uri() . "/assets/img/No_image.png"; }  ?>" onerror="javascript:this.src='<?php echo get_template_directory_uri() . "/assets/img/No_image.png"; ?>'" itemprop="image"  title="<?php the_title(); ?>" alt="<?php the_title(); ?>">
		        </a>
			</figure>
		</div>
		<div class="col-12 col-xl-6 pl-0">
			<header class="sub-recent-header">
			    ...
			    <?php the_title( sprintf( '<h3 itemprop="headline" class="pb-0 categories-title-height h5"><a href="%s" itemprop="url" class="h5 sub-recent-title" title="'.get_the_title().'">', esc_url( get_permalink() ) ), '</a></h3>' ); ?>
	        ...
	        <div class="sub-recent-post-content">
	            <?php echo "<p itemprop='articleBody'>".excerpt(10)."</p>"; ?>
	        </div>
		</div>
	</div>
</article>